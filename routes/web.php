    ...
    $content =
        [
            ...
            "pickupParam" => [
                ...
                "name" => "zhangsan",
                "detailAddress" => "888 Jiangcun Street",
                "telephone" => "0762-88888888",
...
function testWaybill()
{
    ...
    $content = [
        "waybillType" => "256",// 1 for cn23 //256 for cn35
        "orderCode" => "CNCAIPBRSAODDNL45832001150301",
        "locale" => "zh_CN",
...
Route::get('/cnge-airline-receive', function (Request $request) {
    return cngeAirlineReceive();
});
function cngeAirlineReceive()
{
    ...
    $content = [
        ...
        "airlineParam" => [
            "flightNo" => "FT0001",
            "eta" => "2021-11-18 15:46:24",
            "etd" => "2021-11-18 15:46:24",
            "transitPortCode" => "GD",
            "toPortCode" => "LA",
            "fromPortCode" => "SN"
        ],
    ...
    $appSecret = '2A1X6281822ts3068j1F8Wdq99C76119';   // APPKEY对应的秘钥 
    $cpCode = 'de316159604032ef042935f43ffc3e2b';     //  调用方的CPCODE 
    $msgType = 'cnge.airline.receive';  // 调用的API名 
    $toCode = 'CGOP';        //  调用的目标TOCODE，有些接口TOCODE可以不用填写
    $digest = base64_encode(md5(json_encode($content) . $appSecret, true)); //生成签名  
    echo ('digest is ' . $digest);
    $ch = curl_init();
...
if (isset($matches[1])) {
    ...
    if ($data !== null) {
        // URL-decode the cn38Url
        $data['cn38Url'] = urldecode($data['cn38Url']);

        // Rebuild the JSON string
        $new_logistics_interface = json_encode($data);

        // Print the new JSON string
        dd(json_decode($new_logistics_interface));
    } else {
        echo 'Failed to decode JSON.';
    }
} else {
    echo 'Failed to extract JSON portion.';
}
