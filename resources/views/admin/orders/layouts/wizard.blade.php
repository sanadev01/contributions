        ...
        <div class="row">
            <div class="col-12">
                <div class="card">
                    ...
                    @section('title', __('orders.order-details.Orders'))
                    {{-- <a href="{{ route('admin.roles.create') }}" class="pull-right btn btn-primary"> Create Role </a> --}}
                </div>
                @if (in_array(request()->route()->getName(),
                    ['admin.orders.sender.index']))
                    <div class="col-12 m-2">
                        <marquee direction="left"
                            style="font-size: xx-large;color: #fff;background-color: #246bad;border-radius: 0%;">
                            @lang('orders.scrolling')</marquee>
                    </div>
                @endif
                    ...
                    <div class="card-body paddinglr">
                        ...
                        <div class="number-tab-steps wizard-circle wizard clearfix" role="application" id="steps-uid-0">
                            <div class="steps clearfix no-print">
                                <ul role="tablist">
                                    <li role="tab"
                                        class="first {{ in_array(request()->route()->getName(),['admin.orders.sender.index'])? 'current': 'disabled' }}"
                                    ...
                                    <li role="tab"
                                        class="{{ in_array(request()->route()->getName(),['admin.orders.recipient.index'])? 'current': 'disabled' }}"
                                        aria-disabled="true">
                                    ...
                                    <li role="tab"
                                        class="{{ in_array(request()->route()->getName(),['admin.orders.order-details.index'])? 'current': 'disabled' }}"
                                        aria-disabled="true">
                                        <a id="steps-uid-0-t-1" href="#" aria-controls="steps-uid-0-p-1">
                                            <span class="step">3</span> @lang('orders.order-details.Shipping & Items')
                                        </a>
                                    </li>
                                    @if (($order->user->hasRole('wholesale') && $order->user->insurance == false) ||
                                        $order->user->hasRole('retailer'))
                                        <li role="tab"
                                            class="{{ in_array(request()->route()->getName(),['admin.orders.services.index'])? 'current': 'disabled' }}"
                                            aria-disabled="true">
                                    ...
                                    @endif
                                    <li role="tab"
                                        class="last {{ in_array(request()->route()->getName(),['admin.orders.order-invoice.index'])? 'current': 'disabled' }}"
                                        aria-disabled="true">
                                        <a id="steps-uid-0-t-2" href="#" aria-controls="steps-uid-0-p-2">
                                            @if (($order->user->hasRole('wholesale') && $order->user->insurance == false) ||
                                                $order->user->hasRole('retailer'))
