<div class="row">
    <div class="col-md-12 text-right">
        ...
        @if (!$error)
            <button onclick="window.open('@if($order->shippingService->isColombiaService()) {{ $order->colombiaLabelUrl() }} 
            @elseif($order->shippingService->is_total_express) {{ $order->totalExpressLabelUrl() }}
            @else{{ route('order.label.download',[encrypt($order->id),'time'=>md5(microtime())]) }}@endif','','top:0,left:0,width:600px;height:700px;')" class="btn btn-primary">Download</button>
