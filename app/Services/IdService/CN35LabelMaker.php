<?php 
namespace App\Services\IdService;
...
class CN35LabelMaker implements HasLableExport
{
    ...
    public function __construct(Container $container)
    {
        ...
        if($order){ 
              $this->setType($order->getOriginalWeight('kg'), $container); 
        }
    ...
    public function setType(string $weight, $container)
    {
        $this->OrderWeight = $weight; 
        $this->officeAddress = $container->getServiceSubClass(); 
        return $this;
    }
    ...
    {
        return view('labels.id-service.cn35.index',$this->getViewData());
    }

    public function download()
    {
        return \PDF::loadView('labels.id-service.cn35.index',$this->getViewData())->stream();
    }

    public function saveAs($path)
    {
        return \PDF::loadView('labels.id-service.cn35.index',$this->getViewData())->save($path);
    }
