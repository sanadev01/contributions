...
class MileExpressUnitRegisterController extends Controller
{
     ...
     */
    public function __invoke(Container $container, MileExpressContainerRepository $mileExpressContainerRepository)
    {
        if (count($container->orders) > 0){
            $airWayBillIds = $mileExpressContainerRepository->getAirWayBillIdsForMileExpress($container->orders);
